...
1. **CocoaPods Account**: Register at [CocoaPods.org](https://cocoapods.org)
...
```bash
# Validate locally
pod spec lint TLog.podspec

# Validate against CocoaPods servers
pod spec lint TLog.podspec --sources='https://github.com/CocoaPods/Specs.git'
...
pod trunk push TLog.podspec

# If you encounter warnings but want to proceed:
pod trunk push TLog.podspec --allow-warnings
...
```bash
# Update version in TLog.podspec
...
## 📦 Publishing to Swift Package Manager

### Prerequisites

Your repository must be accessible via Git (GitHub, GitLab, etc.)

### Publishing Steps

#### **1. Ensure Package.swift is Correct**
...
let package = Package(
    ...
    products: [
        .library(name: "TLog", targets: ["TLog"]),
    ],
    targets: [
        .target(name: "TLog", dependencies: [], path: "Sources/TLog"),
        .testTarget(name: "TLogTests", dependencies: ["TLog"], path: "Tests/TLogTests"),
    ],
    swiftLanguageVersions: [.v5]
)
```

#### **2. Create Git Tags for Versions**
...
# Add TLog dependency to Package.swift
# Test build
swift build
...
#### **4. Submit to Swift Package Index (Optional)**

1. Go to [SwiftPackageIndex.com](https://swiftpackageindex.com)
2. Click "Add a Package"
3. Enter your repository URL: `https://github.com/fanta1ty/TLog`
...
```bash
# Semantic versioning
git tag 2.0.0      # Major release
git tag 2.1.0      # Minor release  
git tag 2.1.1      # Patch release
git tag 2.2.0-beta # Beta release
...
- 📖 **Documentation**: [API Reference](https://fanta1ty.github.io/TLog/)
...
Made with ❤️ by [fanta1ty](https://github.com/fanta1ty)
...
**Output Example:**
```
14:32:18.742 ℹ️ [INFO] [AUTH] <LoginViewController.swift:45> User authentication successful
```
